...
const DatePicker = React.forwardRef(({ size = "base", shouldCloseOnSelect = true, className, modal = false, ...props }, ref) => {
    ...
    React.useEffect(() => {
        setValue(props.value
            ? (0, calendar_1.updateCalendarDateFromDate)(value, props.value, props.granularity)
            : null);
...
function convertProps(props, setValue) {
    ...
    return {
        ...
        isDateUnavailable,
        minValue: minValue
            ? (0, calendar_1.createCalendarDateFromDate)(minValue, props.granularity)
            : minValue,
        maxValue: maxValue
            ? (0, calendar_1.createCalendarDateFromDate)(maxValue, props.granularity)
            : maxValue,
    };
}
